                        ...
                        auto runAlignmentKernels = [&](auto& d_scores, ReferenceIdT* d_overflow_positions, int* d_overflow_number){
                            ...
                            for(int lp = plan.h_partitionIds.size() - 1; lp >= 0; lp--){
                                ...
                                if(kernelTypeConfig.singlePassType == KernelType::Half2){
                                    ...
                                    if (partId == 0){ call_NW_local_affine_read4_float_query_Protein_new<2>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 1){ call_NW_local_affine_read4_float_query_Protein_new<2>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 2){ call_NW_local_affine_read4_float_query_Protein_new<3>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 3){ call_NW_local_affine_read4_float_query_Protein_new<3>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 4){ call_NW_local_affine_read4_float_query_Protein_new<4>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 5){ call_NW_local_affine_read4_float_query_Protein_new<4>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 6){ call_NW_local_affine_read4_float_query_Protein_new<5>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 7){ call_NW_local_affine_read4_float_query_Protein_new<5>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 8){ call_NW_local_affine_read4_float_query_Protein_new<6>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 9){ call_NW_local_affine_read4_float_query_Protein_new<6>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 10){ call_NW_local_affine_read4_float_query_Protein_new<7>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 11){ call_NW_local_affine_read4_float_query_Protein_new<7>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 12){ call_NW_local_affine_read4_float_query_Protein_new<8>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 13){ call_NW_local_affine_read4_float_query_Protein_new<8>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 14){ call_NW_local_affine_read4_float_query_Protein_new<9>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 15){ call_NW_local_affine_read4_float_query_Protein_new<10>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 16){ call_NW_local_affine_read4_float_query_Protein_new<11>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 17){ call_NW_local_affine_read4_float_query_Protein_new<12>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 18){ call_NW_local_affine_read4_float_query_Protein_new<13>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 19){ call_NW_local_affine_read4_float_query_Protein_new<14>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 20){ call_NW_local_affine_read4_float_query_Protein_new<15>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 21){ call_NW_local_affine_read4_float_query_Protein_new<16>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 22){ call_NW_local_affine_read4_float_query_Protein_new<18>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 23){ call_NW_local_affine_read4_float_query_Protein_new<20>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 24){ call_NW_local_affine_read4_float_query_Protein_new<22>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 25){ call_NW_local_affine_read4_float_query_Protein_new<24>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 26){ call_NW_local_affine_read4_float_query_Protein_new<26>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 27){ call_NW_local_affine_read4_float_query_Protein_new<28>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 28){ call_NW_local_affine_read4_float_query_Protein_new<30>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 29){ call_NW_local_affine_read4_float_query_Protein_new<32>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 30){ call_NW_local_affine_read4_float_query_Protein_new<34>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 31){ call_NW_local_affine_read4_float_query_Protein_new<36>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 32){ call_NW_local_affine_read4_float_query_Protein_new<38>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 33){ call_NW_local_affine_read4_float_query_Protein_new<40>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                
                                }else if(kernelTypeConfig.singlePassType == KernelType::DPXs32){
                                    //single pass ! must not use temp storage! allowed sequence length <= 32*numregs
                                    if (partId == 0){ call_NW_local_affine_s32_DPX_new<2>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 1){ call_NW_local_affine_s32_DPX_new<2>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 2){ call_NW_local_affine_s32_DPX_new<3>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 3){ call_NW_local_affine_s32_DPX_new<3>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 4){ call_NW_local_affine_s32_DPX_new<4>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 5){ call_NW_local_affine_s32_DPX_new<4>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 6){ call_NW_local_affine_s32_DPX_new<5>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 7){ call_NW_local_affine_s32_DPX_new<5>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 8){ call_NW_local_affine_s32_DPX_new<6>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 9){ call_NW_local_affine_s32_DPX_new<6>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 10){ call_NW_local_affine_s32_DPX_new<7>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 11){ call_NW_local_affine_s32_DPX_new<7>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 12){ call_NW_local_affine_s32_DPX_new<8>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 13){ call_NW_local_affine_s32_DPX_new<8>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 14){ call_NW_local_affine_s32_DPX_new<9>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 15){ call_NW_local_affine_s32_DPX_new<10>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 16){ call_NW_local_affine_s32_DPX_new<11>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 17){ call_NW_local_affine_s32_DPX_new<12>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 18){ call_NW_local_affine_s32_DPX_new<13>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 19){ call_NW_local_affine_s32_DPX_new<14>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 20){ call_NW_local_affine_s32_DPX_new<15>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 21){ call_NW_local_affine_s32_DPX_new<16>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 22){ call_NW_local_affine_s32_DPX_new<18>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 23){ call_NW_local_affine_s32_DPX_new<20>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 24){ call_NW_local_affine_s32_DPX_new<22>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 25){ call_NW_local_affine_s32_DPX_new<24>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 26){ call_NW_local_affine_s32_DPX_new<26>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 27){ call_NW_local_affine_s32_DPX_new<28>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 28){ call_NW_local_affine_s32_DPX_new<30>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 29){ call_NW_local_affine_s32_DPX_new<32>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 30){ call_NW_local_affine_s32_DPX_new<34>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 31){ call_NW_local_affine_s32_DPX_new<36>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 32){ call_NW_local_affine_s32_DPX_new<38>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
                                    if (partId == 33){ call_NW_local_affine_s32_DPX_new<40>(blosumType, inputChars, d_scores, nullptr, nullptr, inputOffsets, inputLengths, d_selectedPositions, numSeq, d_query, currentQueryLength, gop, gex, nextWorkStreamNoTemp()); CUERR }
